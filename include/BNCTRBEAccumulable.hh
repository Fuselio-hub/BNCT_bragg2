 ...
 * The class is closely tied with the singleton BNCTRBE that is used both
 ...
 *   2) (Optional) inter-run merging of data (implemented in BNCTRBE).
 *
 * @note std::valarray is used (instead of C arrays or std::vectors)
 *    to accumulate data for its logical simplicity.
 */
class BNCTRBEAccumulable : public G4VAccumulable
{
public:
    BNCTRBEAccumulable();
    BNCTRBEAccumulable(const BNCTRBEAccumulable& other) = default;
...
private:
    ...
    // Apply configuration from the BNCTRBE class and prepare matrices
    void Initialize();
