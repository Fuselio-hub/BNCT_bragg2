...
BNCTLet* BNCTLet::instance = NULL;
G4bool BNCTLet::doCalculation = false;

BNCTLet* BNCTLet::GetInstance(BNCTDetectorConstruction *pDet)
{
    if (instance) delete instance;
    instance = new BNCTLet(pDet);
    return instance;
}

BNCTLet* BNCTLet::GetInstance()
{
    return instance;
}

BNCTLet::BNCTLet(BNCTDetectorConstruction* pDet)
:filename("Let.out"),matrix(0) // Default output filename
{
    
    matrix = BNCTMatrix::GetInstance();
    
    if (!matrix)
        G4Exception("BNCTLet::BNCTLet",
                    "BNCT0005", FatalException,
                    "BNCTMatrix not found. Firstly create an instance of it.");
    ...
    pPGA = (BNCTPrimaryGeneratorAction*)runManager -> GetUserPrimaryGeneratorAction();
    // Pointer to the detector material
    detectorMat = pDet -> GetB10LogicalVolume() -> GetMaterial();
...
void  BNCTLet::FillEnergySpectrum(G4int trackID,
