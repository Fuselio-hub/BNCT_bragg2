...
BNCTPrimaryGeneratorMessenger::BNCTPrimaryGeneratorMessenger(
                                                                               BNCTPrimaryGeneratorAction* BNCTGun)
:BNCTAction(BNCTGun)
{
...
void BNCTPrimaryGeneratorMessenger::SetNewValue(G4UIcommand* command, G4String NewValue)

{
    if (command == calculatedPhaseSpaceFileIN)
    {
        BNCTAction->setCalculatedPhaseSpaceFileIN(NewValue);
    }
    
    if (command == NewSource)
    {
        BNCTAction->setNewSource(NewSource->GetNewBoolValue(NewValue));
