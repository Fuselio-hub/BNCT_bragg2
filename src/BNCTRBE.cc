...
BNCTRBE* BNCTRBE::instance = nullptr;

BNCTRBE* BNCTRBE::GetInstance()
{
    return instance;
}

BNCTRBE* BNCTRBE::CreateInstance(G4int voxelX, G4int voxelY, G4int voxelZ, G4double massOfVoxel)
{
    if (instance) delete instance;
    instance = new BNCTRBE(voxelX, voxelY, voxelZ, massOfVoxel);
    return instance;
}

BNCTRBE::BNCTRBE(G4int voxelX, G4int voxelY, G4int voxelZ, G4double massOfVoxel)
...
void BNCTRBE::LoadLEMTable(G4String path)
{
    ...
    if (!in)
    {
        ...
        G4Exception("BNCTRBE::LoadData", "WrongTable", FatalException, ss.str().c_str());
    }
    ...
    if (!getline(in, line))
    {
        ...
        G4Exception("BNCTRBE::LoadLEMTable", "CannotReadHeader", FatalException, ss.str().c_str());
    }
    ...
    for (auto columnName : columns)
    {
        ...
        if (pos == header.end())
        {
            ...
            G4Exception("BNCTRBE::LoadLEMTable", "ColumnNotPresent", FatalException, ss.str().c_str());
        }
        else
        {
...
void BNCTRBE::SetCellLine(G4String name)
{
    if (fTablesEnergy.size() == 0)
    {
        G4Exception("BNCTRBE::SetCellLine", "NoCellLine", FatalException, "Cannot select cell line, probably LEM table not loaded yet?");
    }
    if (fTablesEnergy.find(name) == fTablesEnergy.end())
    {
        stringstream str;
        str << "Cell line " << name << " not found in the LEM table.";
        G4Exception("BNCTRBE::SetCellLine", "", FatalException, str.str().c_str());
    }
    else
    {
...
std::tuple<G4double, G4double> BNCTRBE::GetHitAlphaAndBeta(G4double E, G4int Z)
{
    if (!fActiveTableEnergy)
    {
        G4Exception("BNCTRBE::GetHitAlphaAndBeta", "NoCellLine", FatalException, "No cell line selected. Please, do it using the /rbe/cellLine command.");
    }
    ...
    if ((Z < fMinZ) || (Z > fMaxZ))
    {
        if (fVerboseLevel > 1)
        {
            ...
            G4Exception("BNCTRBE::GetHitAlphaAndBeta", "", JustWarning, str.str().c_str());
        }
...
void BNCTRBE::ComputeAlphaAndBeta()
{
    if (fVerboseLevel > 0)
    {
...
void BNCTRBE::SetDenominator(const BNCTRBE::array_type denom)
{
    if (fVerboseLevel > 1)
    {
...
void BNCTRBE::AddDenominator(const BNCTRBE::array_type denom)
{
    if (fVerboseLevel > 1)
    {
...
void BNCTRBE::SetAlphaNumerator(const BNCTRBE::array_type alpha)
{
    if (fVerboseLevel > 1)
    {
...
void BNCTRBE::SetBetaNumerator(const BNCTRBE::array_type beta)
{
    if (fVerboseLevel > 1)
    {
...
void BNCTRBE::AddAlphaNumerator(const BNCTRBE::array_type alpha)
{
    if (fVerboseLevel > 1)
    {
...
void BNCTRBE::AddBetaNumerator(const BNCTRBE::array_type beta)
{
    if (fVerboseLevel > 1)
    {
...
void BNCTRBE::SetEnergyDeposit(const std::valarray<G4double> eDep)
{
    if (fVerboseLevel > 1)
    {
...
void BNCTRBE::AddEnergyDeposit(const std::valarray<G4double> eDep)
{
    if (fVerboseLevel > 1)
    {
...
void BNCTRBE::StoreAlphaAndBeta()
{
    if (fVerboseLevel > 1)
    {
...
void BNCTRBE::CreateMessenger()
{
    fMessenger = new G4GenericMessenger(this, "/rbe/");
    fMessenger->SetGuidance("RBE calculation");

    fMessenger->DeclareMethod("calculation", &BNCTRBE::SetCalculationEnabled)
            .SetGuidance("Whether to enable RBE calculation")
            .SetStates(G4State_PreInit, G4State_Idle)
            .SetToBeBroadcasted(false);

    fMessenger->DeclareMethod("verbose", &BNCTRBE::SetVerboseLevel)
    ...
    fMessenger->DeclareMethod("loadLemTable", &BNCTRBE::LoadLEMTable)
            .SetGuidance("Load a LEM table used in calculating alpha&beta")
            .SetStates(G4State_PreInit, G4State_Idle)
            .SetToBeBroadcasted(false);

    fMessenger->DeclareMethod("cellLine", &BNCTRBE::SetCellLine)
            .SetGuidance("Set the cell line for alpha&beta calculation")
            .SetStates(G4State_PreInit, G4State_Idle)
            .SetToBeBroadcasted(false);

    fMessenger->DeclareMethod("doseScale", &BNCTRBE::SetDoseScale)
...
/*
G4bool BNCTRBE::LinearLookup(G4double E, G4double LET, G4int Z)
{
    G4int j;
    G4int indexE;
...
/*
void BNCTRBE::interpolation_onE(G4int k, G4int l, G4int indexE, G4double E, G4int Z)
{
...
G4bool BNCTRBE::interpolation_onLET1_onLET2_onE(G4int k, G4int l, G4int indexE, G4double E, G4double LET)
{
...
/* void BNCTRBE::SetThresholdValue(G4double dosecut)
{
    fDoseCut = dosecut;
}

void BNCTRBE::SetAlphaX(G4double alphaX)
{
    fAlphaX = alphaX;
}

void BNCTRBE::SetBetaX(G4double betaX)
{
    fBetaX = betaX;
}*/

void BNCTRBE::SetCalculationEnabled(G4bool enabled)
{
    fCalculationEnabled = enabled;
}

void BNCTRBE::SetAccumulationEnabled(G4bool accumulate)
{
...
/*
void BNCTRBE::SetLEMTablePath(G4String path)
{
    // fLEMTablePath = path;
    LoadLEMTable(path);
}
...
void BNCTRBE::SetDoseScale(G4double scale)
{
    fDoseScale = scale;
}
...
/*
G4bool BNCTRBE::NearLookup(G4double E, G4double DE)
{
